...
| `arm/e0_turn_characterization.py` | profile NDJSON + `trace.jsonl` (required; +`logs/vllm_metrics.ndjson`, +`logs/vllm-a0.log`) | E0 views: fig1 3-panel per-turn LLM/tool time + signed log2 ratio; fig2 LLM-time CDF; fig3 hit-tokens vs turn + GPU-KV usage vs time; fig3-1 hit/KV from worker log; fig4 prev-turn KV-reuse vs turn gap; `zero_turns.csv` buffered-tool-call diagnostic. CSVs + PDFs |
| `arm/analyze_session_nesting.py` | profile NDJSON + `trace.jsonl` | structure of MAIN sample sessions vs their nested `task` sub-agent sessions: spawning turn, sub-agent fresh-prefix check (first-turn hit ~0), parent resume-turn KV reuse, ordinal interleave. `session_nesting.csv` / `session_summary.csv` / `ordinal_map.csv` |
