...
SLA lines: --ttft-sla-ms / --tpot-sla-ms draw a horizontal line on the
matching figure and, per batch-size curve, mark the longest prompt that
still meets it -- both the largest measured point and a linear
interpolation of the crossing between the bracketing grid points (the
grid is coarse, so the measured value alone understates the true limit).
...
Inputs:
  ...
  --from-csv <path>     skip measurement, re-plot/re-table an earlier run

Outputs (under --out):
  sweep.csv             one row per cell: batch, prompt_tokens, ttft_ms,
                        tpot_ms, decode_ms, total_ms, gen_tokens, error
...
Usage:
  scripts/arm/e9_offline_ttft_tpot_sweep.py --model qwen3-coder-30b-a3b \
      --tp 2 --batch-sizes 1,2,4,8,16 --gen-tokens 128 \
      --ttft-sla-ms 2000 --tpot-sla-ms 50 --out results/e9
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--from-csv", type=Path, default=None,
                    help="re-table/re-plot a previous sweep.csv; no GPU needed")
    ...
    if args.from_csv:
        ...
        if not rows:
            print(f"error: {args.from_csv} is empty", file=sys.stderr)
            return 2
    else:
    ...
    if n_err:
        print(f"\n{n_err}/{len(rows)} cells failed (shown as 'x'); "
              "typically KV OOM at large batch x length")
