...
- **Permissions must be pinned to `allow` or the agent loop hangs (sibling to the question-tool hang).** opencode's permission `evaluate()` (`opencode/packages/opencode/src/permission/evaluate.ts`) returns `{action:"ask"}` as the DEFAULT when no rule matches, and the `write`/`edit`/`bash`/`webfetch` tools call `ctx.ask({permission:...})` which registers a `Deferred` and **blocks on `Deferred.await` indefinitely** (`permission/index.ts:205-209`) until a `POST /permission/:id/reply` arrives — which the headless runner never sends. The classic trigger is the model writing OUTSIDE the workspace (e.g. `/tmp/test_issue.py`): an in-worktree path may match an allow rule, but an out-of-tree path falls through to the "ask" default and hangs until `--task-timeout-s` (pure measurement noise). `deploy/opencode.json.tmpl` therefore ships a top-level `"permission": {"edit":"allow","bash":"allow","webfetch":"allow"}` block so every tool runs unattended. (Immediate override without re-rendering: export `OPENCODE_PERMISSION='{"edit":"allow","bash":"allow","webfetch":"allow"}'` before `up opencode` — `config.ts:710-711` merges it. Restart opencode to pick up either change.) `tests/test_opencode_template.py:test_rendered_template_allows_edit_bash_webfetch_permissions` guards this invariant.
