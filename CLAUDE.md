...
2. **Pre-clone the repo**: `git clone <sample.repo> <abs_dir>` and `git -C <abs_dir> checkout <sample.base_commit>`. Synchronous, fail-fast — done by runner before any OpenCode call. This sidesteps the in-agent `git clone` hang. **Repo cache (default on, `--repo-cache`)**: before the task loop, `run()` calls `warm_repo_cache()` to clone each UNIQUE repo once into `<workspace_root>/.repo-cache/<repo>` (network, retried with backoff); each task then `git clone --local`s from that cache (no network). This collapses N per-task network clones into U (unique-repo) clones — without it, many concurrent clones of the same SWE-bench repos hit GitHub rate-limiting and fail mid-run. `--no-repo-cache` reverts to a direct per-task network clone (also retried). The cache is consulted in `_pre_clone(cache_dir=...)`; a cache miss falls back to direct clone. **Standalone pre-warm**: `python -m testbed warm-cache --split <s> --num-samples <n> --seed <k>` (cli.py) warms the same cache ahead of the run and **exits 1 if any repo failed** — chain it with `&&` before `run` to guarantee zero network clones during the workload (run's own warm step then finds everything cached; `_ensure_repo_cached` is idempotent). Same `(split, seed, n)` must be passed so the deterministic sample pick yields the same repo set. **Workspace pre-clone (default on, `--pre-clone-workspaces`)**: after the cache warm, `run()` assigns ALL workspace directory names up front (`_directory_for`) and clones EVERY task workspace via `prepare_workspaces()` (bounded concurrency 8) BEFORE the first request fires — the workload phase then performs zero clones (`_pre_clone` at task time sees the existing checkout and returns immediately; with `--reset-workspace` it just resets). Workspaces that fail to pre-clone are listed on stderr and retried at their task's arrival (legacy fail-fast path → `error.stage="clone"` if still failing). `--no-pre-clone-workspaces` reverts to per-task clone-at-arrival.
