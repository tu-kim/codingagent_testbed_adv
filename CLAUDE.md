...
Per-session NDJSON layout: one file per `sessionID`, events `query.start, turn.start, llm.start, llm.net, llm.stream-finish, llm.end, tool.start, tool.end, turn.end, query.end`.

**Measurement-trust model** (the reason the patch has a transport layer): client EVENT hooks timestamp when the event was PROCESSED on the event loop — under load, SSE chunks buffer and then drain at once, collapsing event-based LLM brackets to ~ms against multi-second server walls ("buffered turns"). Client events are therefore trustworthy only for (a) client-side work they directly bracket (tool exec — ground truth) and (b) server-provided payloads (tokens, request_id, dynamo). For the LLM wall the patch records, per request: **`llm.end.net`** = `{wall_s, ttfb_s, drain_delay_s, fetch_start_unix_s}` from `Profile.net` — the central provider fetch wrapper (provider.ts) tees the SSE body BEFORE wrapSSE and an eager reader timestamps first/last byte at network pace (immune to consumption lag), keyed by the chunk id's request_id. `drain_delay_s` = streamFinish event − last wire byte = event-loop lag (a direct CPU-contention metric). A standalone `llm.net` event (sessionID from the `x-session-affinity` request header) also fires per request so aborted turns keep their net timing. Preference order for LLM wall: `dynamo.elapsed_s` (server) > `net.wall_s` (transport; = server + upload/network) > event-based `duration_s` (diagnostic only).
...
- **`frontend.log`'s `output_tokens` is TRUNCATED — never use it as per-request OSL** (nor anything derived from it: `avg_itl_ms`, and the Prometheus `output_sequence_length` histogram, which reads the same field). The counting UNIT is fine (real tokens, `cumulative_output_tokens += token_ids.len()`, `preprocessor.rs:2042`); the recorded VALUE is cut. `ResponseMetricCollector::Drop` writes `self.osl` onto the enclosing span (`metrics.rs:1571-1598`), but the cancel path (`disconnect.rs:272-286`, `context.stopped()`) breaks the stream loop early — freezing `self.osl` at a partial value — and the `InflightGuard` that emits the `request completed` line races it with **no guaranteed drop order** (the vendor's own comment at `disconnect.rs:275-276` admits the dependency). Measured 2026-08-06 over a 3006-request run (`scripts/arm/check_osl_truncation.py`): only **34 requests (1.1%) were short — but they were the longest generations**, so **47% of all output tokens vanished** (652797 logged vs 1224838 real) and the frontend's max OSL capped at 18068 against a real 32000. Statuses were 3107 success / 5 error, so the cancel path does NOT explain it — the drop-order race does. Practical shape: frontend-derived MEDIAN OSL/ITL were roughly right; **tails (p90/p99/max), totals, throughput and aggregate ITL were not** (this is where `avg_itl_ms` ~31250 ms comes from — a 62 s decode divided by a 2-token count). **Trusted source = profile `llm.end.tokens.output`** (= dynamo `usage.completion_tokens`, `delta.rs:252-258`, an independent accumulator immune to both failure modes); `nvext.timing` carries no OSL field. `e4_prefill_decode.py --profiles <dir>` (helpers `profile_output_tokens` / `apply_profile_tokens`) applies the correction and preserves the raw value as `output_tokens_frontend`; `e6_kv_capacity.py` applies it automatically when the run has a profiles dir, and both WARN when run without one. Totals-only cross-check: the per-model `..._output_tokens_total` counter (`metrics.rs:1484`) increments per chunk and is immune.
